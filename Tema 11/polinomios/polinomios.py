...
class Polynomial:
    ...
    def __init__(self,*args):
        self.coeficients = self.__removeTrailingZeros(args)
    ...
    def __removeTrailingZeros(self,coeficientes):
        '''
        Elimina los ceros del final en la serie de coeficientes
        coeficientes: tupla con los coeficientes en orden posicional
        resultado: (tuple)
        '''
        coeficiente_list = list(coeficientes)
        while len(coeficiente_list) > 0 and  coeficiente_list[-1] == 0:
            del coeficiente_list[-1]
        if coeficiente_list == []:
            raise ArithmeticError('No hay coeficientes no nulos en la lista')
        return tuple(coeficiente_list)
    
    def __eq__(self, anotherPol):
        return self.coeficients == anotherPol.coeficients
